...
export default function DigitaltInnholdPage() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (activeTab === 'ebøker') {
        url += 'type=books&subType=ebok'
      } else if (activeTab === 'lydbøker') {
        url += 'type=books&subType=lydbok'
      } else if (activeTab === 'film') {
        url += 'type=films'
      }

      if (searchQuery) {
        url += `&søk=${encodeURIComponent(searchQuery)}`
      }
  ...
  const getProviderName = (provider: string) => {
    switch (provider) {
      ...
      case 'filmoteket': return 'Filmoteket'
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-blue-800 text-sm mb-3">
                Alt digitalt innhold er gratis for lånekortinnehavere ved Bergen Bibliotek. 
        ...
        <div className="mb-6">
          <p className="text-gray-600">
            {isLoading ? (
              ...
              <>Viser <strong>{items.length}</strong> {activeTab}</>
            )}
          </p>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <p className="text-gray-600 mb-4">
              Tilgang til tusenvis av norske og internasjonale e-bøker og lydbøker. 
              Lån direkte til din enhet og les/lytt offline.
            </p>
            <ul className="space-y-2 text-sm text-gray-600">
              <li>✓ Ubegrenset utlån</li>
              <li>✓ Les på mobil, nettbrett eller PC</li>
              <li>✓ Automatisk retur etter låneperioden</li>
              <li>✓ Ingen forsinkelsesgebyrer</li>
            </ul>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <p className="text-gray-600 mb-4">
              Stream norske og internasjonale filmer og serier. 
              Stort utvalg av klassikere og nye utgivelser.
            </p>
            <ul className="space-y-2 text-sm text-gray-600">
              <li>✓ HD-kvalitet streaming</li>
              <li>✓ Norsk film og TV-serier</li>
              <li>✓ Dokumentarer og barneprogrammer</li>
              <li>✓ Ingen ekstra kostnader</li>
            </ul>
          </div>
        </div>
      </main>
